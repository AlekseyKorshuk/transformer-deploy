...
class GenerationMixin:
    ...
    def _prepare_model_inputs(
        ...
        if (
            ...
            input_name = self.model.encoder.main_input_name
        else:
        ...
        if self.model._can_retrieve_inputs_from_name(inputs, "inputs_embeds", model_kwargs):
            inputs, input_name = model_kwargs["inputs_embeds"], "inputs_embeds"

        # 4. Only encoder-decoder models can have non `input_ids` input format
        if not self.model.config.is_encoder_decoder and input_name != "input_ids":
            raise ValueError(
        ...
        if inputs is None:
            inputs = self.model._prepare_input_ids_for_generation(bos_token_id, model_kwargs.get("encoder_outputs"))
    ...
    def _can_retrieve_inputs_from_name(
        ...
        if can_retrieve_inputs and inputs is not None:
            raise ValueError(f"Cannot only pass one of {name} and {self.model.main_input_name}")
    ...
    def _prepare_input_ids_for_generation(
        ...
        if self.model.config.is_encoder_decoder and encoder_outputs is not None:
            # make dummy input_ids with value -100, as a sanity check ensuring that they won't be used for encoding
            shape = encoder_outputs.last_hidden_state.size()[:-1]
            return torch.ones(shape, dtype=torch.long, device=self.model.device) * -100

        if bos_token_id is None:
            raise ValueError("`bos_token_id` has to be defined when no `input_ids` are provided.")
        return torch.ones((1, 1), dtype=torch.long, device=self.model.device) * bos_token_id
    ...
    def _prepare_attention_mask_for_generation(
        ...
        if is_input_ids and is_pad_token_in_inputs and is_pad_token_not_equal_to_eos_token_id:
            ...
            return torch.ones(inputs.shape[:2], dtype=torch.long, device=self.model.device)
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        model_input_name = model_input_name if model_input_name is not None else self.model.main_input_name
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if model_kwargs is not None and "decoder_input_ids" in model_kwargs:
            ...
            decoder_start_token_id = self.model._get_decoder_start_token_id(decoder_start_token_id, bos_token_id)
            return torch.ones((batch_size, 1), dtype=torch.long, device=self.model.device) * decoder_start_token_id
    ...
    def _reorder_cache(self, past, beam_idx):
        raise NotImplementedError(
            f"Make sure that a `_reorder_cache` function is correctly implemented in {self.model.__class__.__module__} to enable beam search for {self.model.__class__}"
        )
    ...
    def _get_logits_warper(
        ...
        typical_p = typical_p if typical_p is not None else self.model.config.typical_p
    ...
    def _get_logits_processor(
        ...
        remove_invalid_values = (
            remove_invalid_values if remove_invalid_values is not None else self.model.config.remove_invalid_values
        )
        ...
        processors = self.model._merge_criteria_processor_list(processors, logits_processor)
        return processors
    ...
    def _get_stopping_criteria(
        ...
        criteria = self.model._merge_criteria_processor_list(criteria, stopping_criteria)
        return criteria
    ...
    def compute_transition_beam_scores(
        ...
        beam_sequence_indices = torch.tensor(beam_indices, device=sequences.device) * self.model.config.vocab_size
    ...
    def generate(
        ...
        if is_greedy_gen_mode:
            ...
            input_ids, model_kwargs = self._expand_inputs_for_generation(
                input_ids, expand_size=num_beams, is_encoder_decoder=self.model.config.is_encoder_decoder,
                **model_kwargs
            )
            # 12. run beam search
    ...
    def group_beam_search(
        ...
        while True:
            ...
            if model_kwargs["past"] is not None:
                model_kwargs["past"] = self.model._reorder_cache(model_kwargs["past"], reordering_indices)
